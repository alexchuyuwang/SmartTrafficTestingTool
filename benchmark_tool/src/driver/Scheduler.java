...
public final class Scheduler {
	...
	/**
	 * Gets the inter-arrival times of events If the arrival process is
	 * DETERMINISTIC, T = 1/X. If the arrival process is POISSON, T = -ln(U)/X.
	 *
	...
	/**
	 * Gets the inter-arrival times of events (deterministic: T = 1/X).
	 *
	 * @return The time in nanoseconds
	 */
	private long getDeterministicInterArrivalTime() {
		long interTime = Math.round(1 / getRate());
		this.currentTime = this.currentTime + interTime;
		return interTime;
	}

	/**
	 * Gets the inter-arrival times of events (Poisson Process: inter-arrival
	 * times exponentially distributed).
	 *
	 * @return The time in nanoseconds
	 */
	private long getPoissonInterArrivalTime() {
		long interTime = Math.round(-Math.log(rnd.nextDouble()) / getRate());
		this.currentTime = this.currentTime + interTime;
		return interTime;
	}
